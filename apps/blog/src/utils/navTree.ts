...
/**
 * Same shape as buildDocsTree, scoped to the `learn` collection. Top-level
 * groups (i.e., track folders) also carry an `href` pointing at the track's
 * home page (e.g., `/learn/openliberty/`) — the sidebar uses this so
 * clicking the track name navigates instead of just toggling the disclosure.
 */
export function buildLearnTree(items: LearnItem[], base: string): NavNode[] {
  ...
  for (const item of sorted) {
    ...
    for (let i = 0; i < parts.length - 1; i++) {
      const dir = parts[i];
      const cleanDir = stripPrefix(dir);
      // A "group" is a node with children; match by sortKey + children
      // (the old `!c.href` test rules out top-level track groups now that
      // they carry an href).
      let child = cursor.children!.find(
        (c) => c.sortKey === dir && c.children !== undefined
      );
      if (!child) {
        const isTopLevelTrack = cursor === root;
        child = {
          label: prettify(cleanDir),
          children: [],
          sortKey: dir,
          ...(isTopLevelTrack ? { href: `${base}/${cleanDir}/` } : {}),
        };
        cursor.children!.push(child);
      }
      cursor = child;
