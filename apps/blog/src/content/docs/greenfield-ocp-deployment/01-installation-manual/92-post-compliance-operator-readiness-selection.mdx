...
description: "Read-only operator readiness, parity audit, install/defer decisions, the consolidated v7 operator batch plan, and the spoke Pipelines plus observability foundation executions."
...
`OP-GF-OPERATORS-21` Pipelines Batch 1 execution, and the
`OP-GF-OPERATORS-22` observability foundation Batch 2 execution so the manual
keeps one coherent operator path instead of one sidebar entry per preflight,
...
| 3 | `OP-GF-OPERATORS-23` | `spoke-dc-v7` | Tempo, OpenTelemetry, Service Mesh, and Kiali | Next. Can end as defer/no-install if there is no sample app or mesh onboarding requirement. |
