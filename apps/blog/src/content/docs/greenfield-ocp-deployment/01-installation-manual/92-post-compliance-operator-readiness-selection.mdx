...
description: "Read-only operator readiness, parity audit, install/defer decisions, the consolidated v7 operator batch plan, and the spoke Pipelines Batch 1 execution."
...
`OP-GF-OPERATORS-21` Pipelines Batch 1 execution so the manual keeps one
...
| 2 | `OP-GF-OPERATORS-22` | hub/spoke after preflight | Cluster Observability Operator and NetObserv | Next. Storage, retention, flow volume, and resource posture must be settled before install. |
