...
| RAG ingestion | Exposes reviewed source ingestion, source/job/chunk reads, keyword query citations, `/healthz`, `/readyz`, and `/metrics`. |
...
The RAG ingestion skeleton does not add production-changing authority. It creates contract-valid source, job, chunk, and keyword query behavior for reviewed non-secret fixtures only. Runtime RAG state is currently in process; durable Postgres metadata, MinIO source object handling, embedding generation, and Qdrant indexing remain follow-up work.
