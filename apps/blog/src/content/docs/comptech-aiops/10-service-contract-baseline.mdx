...
Deployment verification also corrected the ingress health route so `/healthz`, `/readyz`, and `/api/*` are served by the portal API rather than the portal web container.
