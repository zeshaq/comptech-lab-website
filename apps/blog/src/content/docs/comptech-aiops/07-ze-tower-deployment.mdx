...
RAG semantic indexing verification should use public-safe reviewed source fixtures only. The smoke scope is ingestion, `indexed` chunk state, Qdrant point metadata, direct and portal-mediated citation queries, and the internal index rebuild endpoint. Secret values and raw private evidence are not published here.
