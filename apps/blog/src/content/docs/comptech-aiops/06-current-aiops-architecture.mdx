...
| RAG ingestion | Internal `/internal/v1/rag` ingestion skeleton with reviewed source ingestion, deterministic chunking, keyword query citations, liveness, readiness, and metrics. |
...
The RAG ingestion skeleton is suitable for contract validation with reviewed, non-secret fixtures. Durable Postgres metadata, MinIO source object handling, embedding generation, and Qdrant indexing remain follow-up work.
