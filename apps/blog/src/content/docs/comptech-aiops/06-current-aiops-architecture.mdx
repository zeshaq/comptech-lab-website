...
| RAG ingestion | Internal `/internal/v1/rag` ingestion service with durable Postgres metadata, reviewed source object storage in MinIO, deterministic chunking, keyword query citations, liveness, readiness, and metrics. |
...
The RAG ingestion path is suitable for reviewed, non-secret fixtures and early knowledge packs. Embedding generation and Qdrant indexing remain follow-up work before retrieval quality should be treated as semantic.
