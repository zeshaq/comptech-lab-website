...
| Operational store | Postgres-backed task records, approval decisions, evidence metadata reads, and task audit events; deployed smoke checks passed on `ze-tower`. |
| Internal clients | Portal API calls orchestrator, MCP gateway, and RAG stubs for readiness, task enqueue/cancel, inventory targets, and knowledge query; deployed smoke checks passed on `ze-tower`. |
