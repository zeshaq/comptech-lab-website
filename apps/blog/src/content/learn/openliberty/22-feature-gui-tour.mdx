...
> **A real customer-facing GUI lives in chapter 23 onward.** Slices 18–24
> build a SvelteKit customer portal and a React agent dashboard, both
> with real OIDC login through WSO2 IS. The vanilla tour stays
> alongside as the demo/teaching surface — different audience.
...
real auth flow. The customer portal in [chapter 23](/learn/openliberty/customer-portal-foundation)
onward does the real OIDC dance — authorization code + PKCE,
HttpOnly session cookie, no JWT in the browser. The vanilla tour
keeps the shortcut on purpose; the audiences are different.
...
Next: [23 — Customer portal foundation →](/learn/openliberty/customer-portal-foundation)
