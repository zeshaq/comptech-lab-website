...
> **What about human users, not just service accounts?** Everything in
> this chapter is the `client_credentials` flow — fine for backend-to-
> backend, useless for a real person logging in. The browser-side OIDC
> dance (authorization code + PKCE, BFF-managed session cookie, no JWT
> in the browser) is its own chapter much later in the track —
> [27 — Real human OIDC end-to-end](/learn/openliberty/real-human-oidc) —
> once the customer-facing SvelteKit app exists to host it.
